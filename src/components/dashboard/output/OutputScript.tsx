...
export default function OutputScript({ longScript, shortScript, caption }: OutputScriptProps) {
    ...
    return (
        <div className={`relative bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-xl p-6 shadow-sm ${isRTL(text) ? "text-right" : "text-left"}`} dir={isRTL(text) ? "rtl" : "ltr"}>
