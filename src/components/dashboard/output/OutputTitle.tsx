...
export default function OutputTitle({ title }: { title?: string }) {
    ...
    return (
        <div className={`relative bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-xl p-6 pb-16 shadow-sm ${isRTL(title) ? "text-right" : "text-left"}`} dir={isRTL(title) ? "rtl" : "ltr"}>
